...
namespace Appccelerate.Bootstrapper.Dummies
{
    ...
    public abstract class CustomExtensionBase : ICustomExtension
    {
        ...
        protected CustomExtensionBase(Queue<string> sequenceQueue)
        {
        ...
        internal void DumpAction(string actionName)
        {
            this.sequenceQueue.Enqueue(string.Format(CultureInfo.InvariantCulture, "Action: {0}", actionName));
        }

        private void Dump(string methodName)
        {
            this.sequenceQueue.Enqueue(string.Format(CultureInfo.InvariantCulture, "{0}: {1}", this.GetType().Name, methodName));
        }
    }
}