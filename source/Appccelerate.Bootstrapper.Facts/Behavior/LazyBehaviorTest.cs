...
namespace Appccelerate.Bootstrapper.Behavior
{
    ...
    {
        private readonly IBehavior<ICustomExtension> lazyBehavior;
        ...
        {
            this.lazyBehavior = A.Fake<IBehavior<ICustomExtension>>();
        ...
        {
            var expectedExtensions = Enumerable.Empty<ICustomExtension>().ToList();

            this.testee.Behave(expectedExtensions);

            A.CallTo(() => this.lazyBehavior.Behave(expectedExtensions)).MustHaveHappened();
        }
